...
def compute_fallacy_metrics(
    gold_fallacies: Dict[int, List[str]], 
    pred_fallacies: Dict[int, List[str]]
...
def compute_reliability_mae(
    gold_scores: Dict[int, int], 
    pred_scores: Dict[int, int]
) -> float:
...
def print_report(metrics: EvaluationMetrics, verbose: bool = True) -> None:
    ...
    if verbose and metrics.fallacy_details:
        ...
        for fallacy_type, scores in sorted(metrics.fallacy_details.items()):
            print(f"   - {fallacy_type}: P={scores['precision']:.2f} R={scores['recall']:.2f} F1={scores['f1']:.2f}")
...
def export_results_csv(results: List[Tuple[str, EvaluationMetrics]], output_path: Path) -> None:
    ...
    with open(output_path, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow([
            "article", "fallacy_precision", "fallacy_recall", "fallacy_f1",
            "reliability_mae", "arg_count_gold", "arg_count_pred"
        ])
